...
ErrorRejectedWLS_m   = NaN(nTrials,1);
...
for ib = 1:nB
    ...
    for mc = 1:N_MC
        ...
        %% --------- Control: WLS after residual rejection ----------------
        % Same residual rejection as the robust procedure, but without BE.
        [thetaRejected,~,~,~] = iterativeWLS( ...
        ...
        errRejected = norm(thetaRejected(1:3)-UserPositionECEF);
        ...
        ErrorRejectedWLS_m(trialRow) = errRejected;
...
for ib = 1:nB
    ...
    eRejected = ErrorRejectedWLS_m(mask);
    eBE   = ErrorRobustBE_m(mask);

    RMSE_FullWLS_m(ib) = sqrt(mean(eFull.^2,'omitnan'));
    RMSE_RejectedWLS_m(ib) = sqrt(mean(eRejected.^2,'omitnan'));
